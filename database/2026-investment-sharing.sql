-- ═══════════════════════════════════════════════════════════════════════════
--  Investments and income become shareable (2026-08-21)
-- ═══════════════════════════════════════════════════════════════════════════
--
-- Adds 'investment' and 'income' to the vocabulary of shareable record types,
-- so a holding or an income entry can be shared to households
-- (`record_households`) and to named people (`record_shares` / `share_codes`)
-- exactly like an account or a loan.
--
-- No new table and no new column: both plug into the SAME join tables every
-- other shareable entity uses, so nothing here can duplicate a holding, a
-- valuation or a pay. Ownership (`user_id` on each row) is untouched — net
-- worth, portfolio totals and TAX are computed from ownership, and sharing
-- only ever records who may LOOK.
--
